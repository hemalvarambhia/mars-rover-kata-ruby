...
class MovingSouthFacingMarsRoverForwardsAndBackwardsTest < MiniTest::Test
  ...
  def test_moving_nowhere
    ...
    expected_location = Location.new(coordinates: Coordinates.new(x: 0, y: -1), direction: 'S')
  ...
  def test_moving_one_step_forward
    ...
    expected_location = Location.new(coordinates: Coordinates.new(x: 2, y: 2), direction: 'S')
    assert_located_at(expected_location, mars_rover)
  end
  ...
  def test_moving_one_step_backward
    ...
    expected_location = Location.new(coordinates: Coordinates.new(x: 3, y: 1), direction: 'S')
    assert_located_at(expected_location, mars_rover)
  end
