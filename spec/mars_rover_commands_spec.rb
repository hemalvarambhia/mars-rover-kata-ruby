describe 'Moving a Mars Rover' do
  ...
  RSpec::Matchers.define :have_moved do
    ...
    failure_message do |mars_rover|
      "expected the Mars rover to have moved #{@direction} " \
        "from #{Position.origin} to #{Position.origin.translate(mars_rover.orientation)} " \
        "but it moved to #{mars_rover.position}"
    end
  end
  ...
  it 'raises error if too many commands are sent' do
    ...
    expect { mars_rover.execute(['f'] * 11) }.to raise_error(TooManyCommands) do |error|
      ...
      expect(error.to_formatted_h).to eq({ message: 'Too many commands', commands: ['f'] * 11 })
    end
