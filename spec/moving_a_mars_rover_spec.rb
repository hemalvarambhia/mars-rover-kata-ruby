  ...
  describe 'Moving forwards' do
    ...
    context 'at the north pole' do
      ...
      it 'moves to the north pole from any line of longitude e.g. (1, 8) to (19, 8)' do
        ...
        expect(mars_rover).to be_located_at(Coordinates.new(x: 19, y: 8)).and be_facing('S')
      end

      it 'moves to the north pole from any line of longitude e.g. (18, 8) to (0, 8)' do
      ...
      it 'moves to the north pole from any line of longitude e.g. (19, 8) to (1, 8)' do
        starting_location = Location.new(direction: 'N', coordinates: Coordinates.new(x: 19, y: 8))
        mars_rover = MarsRover.new(starting_location: starting_location)

        mars_rover.execute(['f'])

        expect(mars_rover).to be_located_at(Coordinates.new(x: 1, y: 8)).and be_facing('S')
      end
  ...
  describe 'Moving backwards' do
    ...
    context 'at the north pole' do
      context 'facing south' do
        it 'moves to the next longitude 180 degrees from the current meridian and faces north'
      end
    end
  end
end
