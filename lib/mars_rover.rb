...
class MarsRover
  ...
  def initialize(initial_position = nil, starting_point: Point.new(x: initial_position[0], y: initial_position[1]), direction: :north)
  ...
  def self.valid_commands?(commands)
    commands.is_a?(Array) &&  !commands.empty? && commands.all? { |command| supported?(command) }
  end
